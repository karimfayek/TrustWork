        ...
        <AuthenticatedLayout>
            <div className="max-w-4xl mx-auto mt-6 p-4 bg-white rounded shadow">
                ...
                <div className="flex gap-2 items-center">
                    <button
                        className="border p-2 my-2 border-red-500"
                        onClick={filterAbsent}
                    >
                        غياب فقط
                    </button>
                    <button
                        className="border p-2 my-2 border-green-500"
                        onClick={filterAtt}
                    >
                        حضور فقط
                    </button>
                    <button
                        className="border p-2 my-2 border-blue-500"
                        onClick={resetFilter}
                    >
                        الكل{" "}
                    </button>
                ...
                <table className="min-w-full table-auto border">
                    <thead>
                        <tr className="bg-gray-100">
                            ...
                            <th className="border px-4 py-2">بواسطة</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredData.map((row, index) => (
                            <tr
                                key={index}
                                className={
                                    "text-center " + TrClassName(row.type)
                                }
                            >
                                ...
                                <td className="border px-4 py-2">{row.by}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
