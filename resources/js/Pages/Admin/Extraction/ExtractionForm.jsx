...
export default function ExtractionForm({
    ...
    supply = false,
    extraction = null,
    DefaultProgressPercentage = null,
    isNotInclusive = false,
}) {
    const [previousQuantities, setPreviousQuantities] = useState(prevQTYs);
    ...
    const handleItemChange = (index, field, value) => {
        ...
        item.total =
            ((item.total_done * item.unit_price) / 100) *
    ...
    const progPercent =
        DefaultProgressPercentage !== null ? DefaultProgressPercentage : 100;
    const deductions =
        defaultDeductions !== null
            ? defaultDeductions
            : {
    ...
    useEffect(() => {
        ...
        if (extraction === null) {
            ...
            const initialItemss = project.tasks.map((task) => {
                ...
                const previous_done =
                    Number(previousQuantities[task.id]) > 0
                        ? Number(previousQuantities[task.id])
                        : 0;
                const current_done =
                    previous_done < quantity ? quantity - previous_done : 0;
    ...
    const handleChange = (e) => {
        ...
        if (name == "type") {
            axios
                .post(
                    `${route("extraction.delevery.getPrevQTYs", [
                        project.id,
                        value,
                    ])}`,
                    {
    ...
    const result = CalcItemsExtraction(
        items,
        form.deductions,
        form.isnotinclusive,
        form.supply
    ...
    return (
        <AuthenticatedLayout>
            <form
                ...
                className="print:hidden space-y-4  mx-auto bg-white p-6 rounded-2xl shadow"
            >
                <div className="bg-blue-50 flex items-center justify-between p-4 sticky top-0">
                    ...
                    <button
                        className="bg-green-100 border border-green-700 px-3 text-2xl"
                        ...
                        {deductionsList.map((deduction) => (
                            <div key={deduction.key}>
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        handleDeductionChange(e, deduction.key)
                                    }
                                />
                            </div>
                        ))}
